...
Demo 阶段的 LLM 应用往往就是一次 `client.chat.completions.create()` 调用。要撑住真实流量，这一次调用的前后会长出一整条链路。本章给出一张贯穿全主题的参考架构图，后续每一章都在填充图中某一个方框的细节。
...
这张图不是某一家公司的具体实现，而是把 OpenAI、Anthropic 官方生产指南和大厂工程博客（Netflix、Uber、Stripe 等）里反复出现的模块抽象出来的**共性骨架**。下面按图中链路顺序过一遍每个方框对应本主题的哪一章。
...
> **一句话概括:生产架构的价值不在画出多少个方框,而在于让「一次调用出问题」的影响半径,被网关、校验、可观测性和评测这几层各自兜住一部分,不至于一路穿透到用户。**
