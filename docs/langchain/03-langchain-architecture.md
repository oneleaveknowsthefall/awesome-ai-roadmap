...
## 3.1 LangChain 到底解决了什么

**直接调用一家模型厂商的 SDK 并不困难。** 真正的问题出现在应用逐渐复杂之后：
...
**最值得理解的不是类名数量，而是一条数据如何从用户走到模型、再进入业务系统。**
...
> **真正执行 Python 函数或外部服务的仍是应用程序，权限校验和副作用也必须留在这里。**
...
> **可信用户身份不需要让模型生成，数据库连接也不会被写入对话上下文。**

工具可以通过 Runtime 读取这些数据，**同时只把真正需要模型填写的参数暴露在工具 Schema 中**。
...
> **LangChain 提供高层组件和标准 Agent 架构，LangGraph 提供底层执行能力。**
>
> 简单 Agent 直接用 `create_agent`；只有流程需要复杂分支、并行、审批或精细状态控制时，才需要直接编写 LangGraph。
...
> **一句话概括：LangChain v1 的架构可以顺着一次请求读完——协议层统一了传什么和怎么执行，Agent 层组装出模型与工具的循环，LangGraph 让这个循环拥有状态、检查点和恢复能力，Middleware 则在每个关键位置留好扩展口。**
