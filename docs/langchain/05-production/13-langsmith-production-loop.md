...
> **生产闭环不是「看到失败就改 Prompt」：Trace 脱敏 → 归因与标注 → 数据集 → 离线实验 → 发布门禁 → 线上评测与反馈 → 失败 Trace 回归。**
...
| 场景 | 采样建议 |
|---|---|
| 安全、拒绝、审批、支付、删除、Tool error | 100% 记录并优先评测；仍应脱敏 |
...
> 不要把单次点赞当质量证明，也不要让用户文本直接改变系统 Prompt 或授权策略。反馈是评测与改进的输入，仍要经过确定性安全控制和人工审核。
...
### 13.7.4 全量运行昂贵 judge 或随机采样高风险路径

为高风险和失败路径设定向、100% 采样；低风险流量再随机采样并设置成本上限。
...
> **一句话概括：LangSmith 的价值不在于多看几条 Trace，而在于把经过脱敏的生产证据持续转成 Dataset、离线实验、发布门禁和线上监测，让每一次真实失败都变成下一次发布前可阻止的回归。**
