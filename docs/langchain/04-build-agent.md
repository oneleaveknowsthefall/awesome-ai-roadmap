...
> **完整 Agent 不是一次模型调用，而是一条从任务设计、能力接入、运行控制走到测试监控的工程链路。**
...
> **模型负责判断和规划，真正访问数据库、搜索资料、发送消息的动作应该由 Tool 执行。**
...
> **工具描述里的「只读」很重要，但 Prompt 中写「禁止退款」不能替代退款接口本身的身份校验。**
...
**底层执行流程**（详见 [第三章](03-langchain-architecture.md)）：
...
> **一句话概括：用 LangChain 构建 Agent 的关键不在于把 create_agent 跑通，而在于沿着「边界、能力、约束、组装、状态、交互、验证」七步，把一个概率性的模型循环变成有边界、有状态、可测试、可观测的业务系统。**
