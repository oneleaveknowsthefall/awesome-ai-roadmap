...
这不是 $\mathrm{pass@}k$，也不写成含糊的 “pass hat k”。对于无人值守生产 Agent， $\mathrm{pass}^{k}$ 补充衡量重复执行的可靠性。tau-bench 的实验揭示： $\mathrm{pass@}1$ 看起来不错时， $k$ 增大， $\mathrm{pass}^{k}$ 仍可能急剧下降，不能把单次平均成绩当成连续运行承诺。
