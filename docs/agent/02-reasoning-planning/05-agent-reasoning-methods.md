...
它的重要性在于提供基线。如果直接调用已经能稳定完成任务，就没有必要引入多轮推理、搜索或 Agent 循环。
...
> 本节聚焦 CoT 的推理机制与局限；CoT 与行动规划的边界见[第十一章](11-llm-agent-planning.md)，可记录的轨迹不等于隐藏 CoT，见[第十四章](../05-production/14-agent-evaluation.md)。
...
Tool-Augmented Reasoning 的关键不只是“允许调用工具”，还包括：
