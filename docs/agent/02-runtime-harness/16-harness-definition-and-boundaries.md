...
这不是咬文嚼字。同一个 Claude 或 GPT 模型，套上不同的 harness，会得到完全不同的产品：命令行里的 Claude Code、IDE 里的 Copilot、云端异步跑的 Coding Agent、Slack 里的客服机器人——模型可能相同，甚至系统提示词也高度相似，但可靠性、可恢复性、可审计性天差地别。差异几乎全部来自 harness 这一层：它怎么组织 Agent Loop、怎么装配上下文、怎么调度工具、怎么隔离风险、怎么处理失败、要不要停下来问人、怎么记录和计费。本模块（第 16–23 章）专门讲这一层，是对第一章 2.12 节 "Runtime 与 Guardrails" 一带而过内容的完整展开。
