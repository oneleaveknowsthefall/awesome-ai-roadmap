...
即便文件系统和进程被隔离，只要沙箱仍然可以任意访问外网，第十五章 15.5 节的"致命三要素"（私有数据 + 不可信输入 + 对外通信能力）依然成立——网络出口是"对外通信"能力最直接的载体。运行时隔离必须能够对出网请求做**默认拒绝、按需放行**的策略,而不是"能不能上网"这种二选一开关。GitHub Copilot Coding Agent 的做法是为每次任务运行提供一个可配置的防火墙，默认限制出网目的地,组织可以显式放行特定域名(见 20.8 节)。这条控制和 20.4 节的进程隔离是互补关系:进程隔离防止"逃出沙箱",网络出口控制防止"沙箱内部把数据发出去"。
