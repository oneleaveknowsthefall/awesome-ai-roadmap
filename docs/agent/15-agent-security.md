...
**核心结论要先说出来**：Agent 安全的主战场不在「让模型更听话」，而在**架构与权限设计**。因为下一节要讲的根本原因，模型层面的防御在原理上就不可能做到完备。
...
这不是当前模型不够强导致的缺陷，而是「用自然语言同时承载指令和数据」这一设计的结构性后果。**任何声称「用一段 Prompt 就能彻底防住注入」的方案都是错的**，可以直接判定为不可信。
...
**这才是 Agent 场景的主要威胁。** 攻击载荷藏在 Agent 会读取的第三方内容里，用户完全不知情。
...
**核心原则：不可逆操作必须有人工确认，这是最后一道也是最可靠的一道防线。** 确认界面上必须展示**实际参数**（收件人是谁、金额多少），而不只是工具名——攻击往往就藏在参数里。
...
API Key、Token 一律由执行层从密钥管理服务取用，**永远不出现在模型的 Context 里**。模型只传逻辑参数。这样即使 Context 被泄露，凭据也是安全的。
...
Agent 安全的核心逻辑可以概括成一条推理链：

> **LLM 无法区分指令与数据 → 读入不可信内容就可能被劫持 → 因此不能靠模型自律 → 必须靠架构、权限与隔离来保证「即使被劫持也造不成损害」。**
...
5. **把 Agent 间消息与工具返回值一律视为不可信输入**；
6. **持续评估**：同时报告 ASR 与 Utility，安全违规一票否决。

一句话概括：

> **不要试图让模型永远不被骗，而要让模型被骗之后也做不成坏事。**
