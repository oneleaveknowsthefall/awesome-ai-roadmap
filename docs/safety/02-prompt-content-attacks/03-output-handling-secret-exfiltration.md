...
模型输出之所以危险，是因为它同时具备两个特征：**内容可被攻击者间接操纵**（通过 Prompt Injection 或越狱），**又天然被下游系统当作「AI 生成的正常结果」而降低审查力度**。这一章覆盖输出处理的具体控制点，以及一种特别值得单列的后果——密钥与敏感数据外泄（secret exfiltration）。
...
Agent 场景下，模型输出不只是展示文本，还会直接成为工具调用的参数。若下游工具对参数「因为来自模型」而信任度过高，就会重现 [Tool Protocol 安全 15.3.2](../../tools/02-mcp/15-tool-protocol-security.md) 描述的问题。这部分的权限与校验设计已在 [Agent 安全](../../agent/05-production/15-agent-security.md) 和 [Tool Protocol 安全](../../tools/02-mcp/15-tool-protocol-security.md) 讲清楚，本章不重复，只强调一点：**服务端重新校验参数，不能相信「这是模型自己算出来的」**。
...
3. 图片自动加载、隐藏字符编码、分批外带、响应时序差异都是隐蔽外带通道，防御核心是限制上下文中的敏感数据而不是穷举通道；
...
> **一句话概括：把模型输出当成攻击者可能部分操纵的「用户输入」来处理，是输出侧安全的全部逻辑起点。**
