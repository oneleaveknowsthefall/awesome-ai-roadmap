...
三者常常组合出现：一次越狱可能同时是为了让模型「忘记」系统提示里的限制，其结果又可能顺带泄漏系统提示内容。架构级的根因——**模型无法从结构上区分指令与数据**——已在 [Agent 安全 15.2](../../agent/05-production/15-agent-security.md) 讲清楚，也是 OWASP LLM01（Prompt Injection）的核心描述；这里直接看三类攻击各自的技术分类，以及系统提示泄漏该怎么单独处理。
