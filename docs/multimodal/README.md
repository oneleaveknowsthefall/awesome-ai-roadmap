...
内容以截至 2026-09-08 可核验的原论文和官方文档为依据。代表模型用于解释机制，不作为最新型号或性能排名；未公开架构不根据产品名称推断。复习时应能说明输入输出、训练信号、失败条件，以及用什么对照实验验证自己的判断。
...
本主题原创文档与示意图：Polo Li，采用 CC BY 4.0；引用论文、模型与外部资料的权利归各自权利人。
