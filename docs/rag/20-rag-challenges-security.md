...
- **最小权限与人工确认**：**RAG 一旦连接工具（发邮件、调 API），风险会成倍放大**；检索内容不应直接触发高权限操作。

**关键不是把材料“提示”为数据，而是让能力和数据流真正受限。** 纯问答型 RAG 仍可能造成错误决策或泄漏；一旦接入 Agent 工具，注入还可能导致真实破坏性操作。Agent 的隔离与授权模式见[Agent 安全](../agent/15-agent-security.md)。
...
- [ ] `doc_id` 已建索引，支持批量删除
- [ ] 更新流程为版本化 copy-on-write，校验后原子切换 alias，并保留可回滚版本
- [ ] 有定期全量哈希对账
...
- Agent 侧的能力隔离、数据流控制和工具授权，见[Agent 安全](../agent/15-agent-security.md)。
- 引用、时效与鲁棒性评测，见[RAG 评估](18-rag-evaluation.md)。
