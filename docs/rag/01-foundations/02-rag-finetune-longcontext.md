...
被问到「RAG 和微调怎么选」时，最容易踩的坑是**二选一思维**——把它答成一道单选题，说完 RAG 的优点就结束了。

2026 年这道题还多了一个维度：模型上下文窗口已经足够大，很多人会直接问「既然能塞进去，为什么还要检索」。

所以现在的正确答法是**三方比较**，并且最终落到「组合使用」而不是「选一个」。
...
**这个区分很重要**：长上下文和 RAG 并不是同一层次的对立选项，RAG 多出来的是「筛选」这一步。所以真正的问题不是「要不要 RAG」，而是**「需不需要在把材料给模型之前先筛一遍」**。
...
**最后一条是被严重低估的价值**：微调一个小模型专做某个任务，长期推理成本可能远低于持续调用大模型。
...
**「不可溯源」和「无法做权限隔离」这两条，是很多企业最终必须用 RAG 的决定性原因**，而不是效果好坏的问题。

### 2.3.3 一个容易答错的点

「用微调给模型灌知识」这件事不是完全做不到，而是**性价比极低且不可控**。面试时不要说「微调学不到知识」，正确说法是：
...
**所以不要一上来就否定长上下文。** 如果你的知识库只有几十篇稳定文档，直接全塞往往比搭一套 RAG 更快更好。
...
前三条是显而易见的工程约束，**第四条才是关键，也是最多人不知道的**。
...
**它的直接推论是：无脑塞更多材料不但不会提升效果，反而可能降低效果。** 这条结论同时也否定了 RAG 里另一个常见做法——见 2.6 节。
...
判断顺序可以总结为四个问题：**装不装得下 → 更不更新 → 要不要过滤 → 调用量大不大**。任何一个答案指向 RAG，就用 RAG。
...
**「微调解决怎么说，RAG 解决说什么」是这道题最值钱的一句话**，因为它一次性说清了两者的正交关系——它们改的根本不是同一个东西，所以不存在互斥。
...
1. **三者改的不是同一个东西**：微调改模型行为，长上下文和 RAG 都是提供材料，区别在于 RAG 多了「筛选」这一步；
2. **微调擅长**格式、风格、领域表达和小模型降本，**不擅长**灌事实、频繁更新、溯源和权限隔离；
3. **长上下文在语料小且稳定时是最优解**，但受规模、时效、权限、质量四个约束；
4. **上下文腐化是真实存在的**：长度增加会持续降低有效利用率，「大海捞针」满分不等于长上下文可靠；
5. **长上下文不能取代 RAG**，两者互补：RAG 负责筛，长上下文负责读；
6. **Top-K 不是越大越好**，必须按模型和任务实测调优；
7. **判断顺序**：装不装得下 → 更不更新 → 要不要过滤 → 调用量大不大；
8. **组合才是常态**：微调解决怎么说，RAG 解决说什么。

一句话概括：

> **微调改的是模型，RAG 改的是材料，长上下文改的是材料的给法。它们正交，所以答案永远是组合，不是选择。**
