...
**注意第 0 步和第 9 步**——这两步在基础版 RAG 里通常没有，但在生产系统里往往是必需的。
...
无脑对所有输入都检索有两个代价：**每次都多花几百毫秒和一次向量化成本**，以及**给闲聊类问题塞进无关文档，反而让答案变差**（第二章 2.4.2 节）。
...
**关键工程注意事项**：**改写会引入延迟（一次 LLM 调用）和风险（改错了方向就全错了）。** 常见的稳妥做法是**同时保留原始 Query 和改写后的 Query 一起检索**，这样即使改写失败，原始 Query 仍能兜底。

**多轮对话场景下，指代消解几乎是必需的。** 直接拿「它多少钱」去检索，必然什么都搜不到。

### 10.3.3 第 2 步：Query 向量化

技术上最简单，但有**整条链路最重要的一条硬性约束**：

> **必须使用与建库时完全相同的 Embedding 模型和版本。**

不同模型的向量空间不同，跨空间计算距离没有意义。而且**这个错误不会报错**——维度对得上就能算出数字，只是结果全是噪音。

一个实用做法是：**把 Embedding 模型的标识写入索引的元数据，服务启动时校验当前配置与索引记录是否一致，不一致直接拒绝启动。** 这比事后排查便宜得多。
...
- **要设分数阈值**。低于阈值的候选**直接丢弃而不是硬凑够 K 个**——这是拒答能力的基础。

### 10.3.7 第 6 步：上下文裁剪与组装

拿到精排结果后，还有三件事要做：

**（1）控制总量。** 按第二章 2.6 节的结论，**不要把所有候选都塞进去**。设定一个 Token 预算，超出就截断。

**（2）安排顺序。** 考虑到「lost in the middle」现象（第二章 2.4.2），**把最相关的内容放在开头和结尾**，次要的放中间，是一个成本为零的优化。

**（3）附加元数据。** 每个片段前标注来源、章节、日期。这有两个作用：让模型知道信息的**时效和权威性**，以及**为引用标注提供依据**。
...
**优化的正确顺序是从耗时最大的开始**：LLM 生成 → Rerank → Query 改写。**在向量检索上抠几毫秒是无效优化。**
...
> **原则：宁可返回一个诚实的「暂时无法回答」，也不要返回一个编造的答案。** 前者是可接受的体验，后者会摧毁用户对系统的信任。
...
即使所有候选都不相关也强行塞满，这是幻觉的直接来源。应该设阈值并允许返回空。
...
6. **Rerank 要设分数阈值，允许返回空**，这是拒答能力的基础；
...
> **在线链路的设计目标不是"每一步都最优"，而是"任何一步失败时，系统仍然给出一个诚实的结果"。**
