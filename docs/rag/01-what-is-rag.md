...
> **幻觉不是一个和前两者并列的独立问题，而是前两者的副产品。**

模型被要求回答一个它没有依据的问题时，它不会说「我不知道」，而会基于语言模式生成一个看起来合理的答案。缺少依据是因，编造答案是果。

这一点在面试里很关键：如果被问「RAG 是不是专门用来解决幻觉的」，正确的回答是**RAG 解决的是知识缺失这个根因，幻觉率下降是随之而来的结果**；而且下一步必须补充——**RAG 只能降低幻觉，不能消除幻觉**（原因见第十七章）。
...
- **需要多跳关系推理的问题**：「A 公司的供应商的竞争对手有哪些」——向量检索是单跳的（第十六章展开）；
