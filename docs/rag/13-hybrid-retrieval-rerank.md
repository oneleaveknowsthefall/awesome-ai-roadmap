...
其中 $R$ 是所有召回路，$\text{rank}_r(d)$ 是文档 $d$ 在第 $r$ 路结果中的排名，$k$ 是平滑常数，**常用值为 60**。
...
**但即便有这些局限，RRF 从提出至今仍是混合检索融合的事实标准**，因为它的鲁棒性和零调参特性在工程上价值极高。
...
这是**最容易被忽略、但对防幻觉最关键**的一点：

> **Rerank 之后要按绝对分数过滤，而不是无条件取 Top-K。**

如果所有候选的相关性分数都很低，说明**知识库里根本没有这个问题的答案**。此时正确的行为是**返回空并拒答**，而不是硬凑够 5 条不相关的内容塞给模型——那是在主动制造幻觉。

**（3）Rerank 分数不能跨 Query 比较。**

不同 Query 的分数分布不同，阈值需要**在评测集上统计确定**，而不是拍脑袋定一个数。
...
**这是最严重的一条**。不设阈值就等于放弃了拒答能力，是幻觉的直接来源。

### 13.7.5 用固定阈值跨 Query 比较

分数分布因 Query 而异，阈值必须在评测集上统计确定。
