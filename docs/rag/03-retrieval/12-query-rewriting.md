...
这里的关键是：假想文档的内容不必正确。它不是用来回答问题的，而是用来**提供检索信号**。真正有用的是它的措辞、结构和术语分布。
...
有些具体问题在文档里并没有直接写出答案，但**支撑推理的原理性内容是有的**。这时先检索原理，再让模型基于原理推导，通常比硬查具体表述更有效。
...
多跳问题可以看作分解的特例：「A 公司 CEO 的母校在哪」需要先查出 CEO 是谁，再查这个人的母校——**后一步依赖前一步的结果，必须串行**。这已经进入 Agentic RAG 的范畴（第十五章）。
...
3. **再按需选一种改写方法**，不要叠加多种，因为每种都会增加一次 LLM 调用；
4. **始终保留原始 Query 一起检索**。
...
4. **HyDE 的做法**：用形式上像文档的假想答案检索，把「问题 vs 文档」变成「文档 vs 文档」；**内容不需要正确，只需要像**；
