...
| 检索结果缓存 | Query → chunk ID 列表 | Query、租户、权限版本、查询过滤、索引快照与检索配置一致 | 更新、删除、撤权时失效，命中后仍校验授权 |
| 语义缓存 | 相似 Query → 答案 | 相似度加实体、日期、数值与权限约束一致 | 高相似度不能证明答案可复用 |
| Prompt Caching | 重复输入前缀 | 满足提供方的前缀、长度、有效期等条件 | 检查写入/读取计费及隐私约束，不是无条件纯收益 |
