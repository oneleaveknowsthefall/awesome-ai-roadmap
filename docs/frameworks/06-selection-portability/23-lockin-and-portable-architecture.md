...
这张图不是唯一答案，而是把本主题讨论过的判断标准串成一条路径。真实项目往往需要同时满足多个条件，例如既要长时间运行又要多智能体协作；这种情况下更常见的是**组合而非二选一**：用 LangGraph 做整体状态编排，在某个节点内部再调用 AutoGen 或 CrewAI 完成一段多智能体协作子任务。这和 [LangChain 生态 · 第七章](../01-langchain/03-ecosystem/07-langchain-vs-llamaindex.md) 展示的「用 Tool 做边界」是同一种思路。
