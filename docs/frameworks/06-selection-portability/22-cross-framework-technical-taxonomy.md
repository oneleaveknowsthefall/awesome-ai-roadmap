...
## 22.1 为什么不能按「产品清单」比较框架

前五个模块已经分别讲过每个框架的核心抽象。如果把它们当作一份产品清单来记忆——「LangChain 是通用 Agent 框架」「LlamaIndex 是 RAG 框架」「AutoGen 是多智能体框架」——很快就会遇到问题：**几乎每个框架都在往其他框架的能力范围扩张**（LlamaIndex 有 Agent 和 Workflows，LangChain 有完整的检索组件，Semantic Kernel 既能做流程编排又能做多智能体协作）。功能清单式的比较会不断过时,也无法回答真正重要的工程问题：**如果今天选了 A，明天需要迁移到 B，代价有多大？**
...
> **判断标准**：如果调试时需要经常回答「第 N 步执行时,系统里到底有哪些数据」，选显式共享状态模型（LangGraph、Process Framework）；如果更看重扩展新步骤的便利性、愿意接受调试成本，隐式事件驱动模型（Workflows、Flow）更合适。
...
> **持久化能力不是「有没有」的二元判断，而是「恢复粒度」的问题**——恢复粒度越细，需要保存和维护的状态版本兼容性成本也越高。选择长时间运行、需要人工审批介入的场景（如 [LangGraph 核心优势](../01-langchain/04-langgraph/10-langgraph-advantages.md) 讨论的人工介入模式）时，这个维度应该被列为选型的硬性约束，而不是「以后再说」的可选项。
...
> **凡是「Schema 生成方式基于标准类型注解」的框架，工具定义本身的可移植性都比较好**——同一个 Python 函数配合类型注解，理论上可以被 LangChain、LlamaIndex、PydanticAI 三者中的任意一个直接消费,这也是这三者经常出现在同一个系统里互相调用的技术基础。真正锁定你的往往不是工具本身，而是**工具调用的编排逻辑**（谁来决定调用顺序、怎么处理调用失败）。
...
**行业层面正在收敛的信号是 [OpenTelemetry GenAI 语义约定](https://github.com/open-telemetry/semantic-conventions-genai)**——它试图为「模型调用」「Agent 步骤」「工具调用」定义统一的 Span 命名和属性规范，让不同框架产生的 Trace 能被同一套观测后端（不限定某个框架自带的 UI）消费。**评估一个框架的可观测性时，除了看它自带的 UI 好不好用，更应该看它是否遵循这套开放标准**——这直接决定了未来更换可观测性后端（甚至更换编排框架本身）时,监控体系能保留多少投入。
...
> **一句话概括：比较框架的正确方式,是把「状态怎么建模」「怎么持久化和恢复」「工具契约怎么生成」「怎么评测和观测」拆成四个独立的技术维度分别打分，而不是罗列它们各自支持哪些功能——这四个维度的组合，才真正决定了一个系统未来的可维护性和可迁移性。**
