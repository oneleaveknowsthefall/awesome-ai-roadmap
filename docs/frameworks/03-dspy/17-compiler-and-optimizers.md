...
第十六章讲的 `Signature` 和 `Module` 只是声明式编程的前半段；DSPy 真正的杀手锏是**编译**——给定一个 `Program`、一个训练/验证数据集和一个评估指标，编译器会自动搜索能让指标最优的具体实现（Few-shot 示例、Prompt 措辞，甚至更新底层模型权重），而不需要人工反复试错。
...
这正是 [评测与选型主题](../../llm/README.md) 中「能力评测指标」一章讨论的评测方法论，在 DSPy 里被反过来当成了**优化目标**，而不只是事后打分工具。
...
> **编译产物替换原 `Program` 时，调用方代码完全不需要修改**——`compiled_program = optimizer.compile(program, trainset=trainset)` 之后，`compiled_program` 和 `program` 暴露的调用接口是一致的，优化只发生在内部的 Prompt 和示例上。
...
1. **DSPy 编译的本质是指标驱动的自动搜索**：给定 `Program`、数据集和评估指标，优化器搜索能让指标最优的具体实现；
