...
[第六章](06-mcp-vs-function-calling.md) 讲过，很多 Host 会把 Server 的工具定义转换成模型原生的 Function Calling 格式。这条**模型驱动**的桥接路径依赖模型接口；若该接口不可用，Host 可以改用结构化输出、规则工作流或人工触发 `tools/call`。MCP 本身并未要求模型具备 Function Calling。
...
> **一句话概括：某些推理模型或 API 曾限制工具调用；这会影响依赖该接口的 Agent 循环，而 MCP 仍可由 Host 用其他受控方式调用。**
