...
这个定位是理解网关所有功能的基础：**它集中拦截了所有出入流量，所以能在这个位置统一做很多事。**
...
这五个痛点看起来五花八门，但源自同一件事：

> **没有一个集中的地方来统一管理这些「横切关注点」。**
...
**「换模型」这件事对业务层彻底隐形了**——你可以在不碰任何业务代码的情况下做 A/B 测试、成本优化、模型迭代。
...
> 语义缓存和 [Prompt Caching](../agent/10-agent-memory-compression.md) 不是一回事：前者是「跳过整次调用」，后者是「调用照做但复用已计算的 KV」。
...
这是冰山一角。要能系统性说出：统一接口、故障转移、Key 集中管理、按团队配额、成本追踪、安全过滤、语义缓存。
...
> **一句话概括：LLM 网关的价值不在「转发请求」，而在于把 Key 管理、配额、成本、安全、缓存这些每个服务都要重复面对的问题，一次性收敛到一个能读懂 token 和语义的位置上。**
