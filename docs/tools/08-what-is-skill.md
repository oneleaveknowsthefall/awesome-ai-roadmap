...
把 Prompt 写进共享文档让大家复制？这本质上还是靠人工维护和执行。文档更新了但有人还在用旧版，质量把控根本做不到。
...
和普通 Prompt 的区别就很清楚了：**Prompt 是一段文字，用完就没了；Skill 是一个可以持续维护、纳入版本管理、全团队共享同一份的完整目录。**

## 8.3 渐进式加载：Skill 最聪明的设计

Skill 真正有意思的地方不是「能打包」，而是**它怎么被加载**。
...
这一点和 [Agent 的上下文压缩](../agent/10-agent-memory-compression.md) 是同一个思路：不是能塞多少就塞多少，而是让模型在恰当的时候只看到恰当的东西。

### 8.3.4 description 决定 Skill 能不能被用上

既然第一层只加载 `description`，那么**这一句话就是 Skill 唯一的曝光机会**。写得含糊，Agent 就发现不了它。
...
这和 [第三章](03-tool-schema-design.md) 里工具 `description` 的写法要求完全一致——都是模型在做选择判断时唯一能看到的信息。
...
**Skill vs Tool**：Tool 提供能力，Skill 提供用这些能力的方法。给新人配了电脑和所有系统权限，他也不知道该按什么流程做代码审查、先查什么后查什么、用什么格式输出。**两者互补，不是替代**。

**Skill vs Slash Command**：都是把指令保存下来复用，但 Slash Command 必须你手动输入 `/code-review` 才会触发；Skill 能被 Agent **自动发现**——它看到任务后自己判断该用哪个 Skill，主动加载执行。这个自动发现能力就是 Skill 比 Slash Command 更进一步的地方。

## 8.5 Skill 里可以放可执行脚本

`scripts/` 目录是个容易被低估的设计。
...
这体现了一种设计思路：**能确定性完成的部分交给代码，需要判断的部分交给模型**。Skill 提供了把两者打包在一起的载体。
...
**为什么能开放**：因为设计足够简单。一个 Skill 就是一个文件夹加一份 Markdown，不需要特殊运行时，不需要学新语言，任何支持文件系统的 Agent 平台理论上都能实现。这种零门槛设计让它有机会成为跨平台约定。

**开放的意义**：你写一份 Skill，未来有机会在不同 Agent 平台之间复用，而不是被某一家产品绑死。目前主要还是 Claude 生态在用，社区有项目在探索跨平台兼容，最终能走多远要看行业采纳。
...
4. **本质是 context 工程**：省 token 是表象，避免注意力被无关内容稀释才是关键；
...
> **一句话概括：Tool 给 Agent 配电脑，Skill 给 Agent 发操作手册，而这份手册聪明在只有翻到需要的那一页时才会真正占用它的注意力。**
