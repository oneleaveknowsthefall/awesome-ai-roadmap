...
标准 HTTP 是「一问一答」模型：客户端发请求，服务端返回响应，连接关闭（或进连接池等待复用）。**服务端在任何时候都不能主动推数据**，只能被动等客户端来问。

这在传统 Web 里够用，但 AI 场景不行：模型生成一个完整回答需要几秒到十几秒，等全部生成完再一次性返回，用户只能干瞪着空白屏幕。我们要的是**生成一个词推一个词**，像 ChatGPT 那样逐字「打」出来。

这需要连接保持打开、服务端能主动持续推送——HTTP 的「问一答一然后断」做不到。

## 13.2 SSE：用普通 HTTP 撑开一条单向水管

### 13.2.1 它不是新协议

SSE（Server-Sent Events）本质上是对 HTTP 的一种**巧用**，是 HTTP/1.1 里本来就有的特性，只是以前很少用。
...
传语音或图片必须先 Base64 编码，**数据量膨胀约 33%**，接收端还要额外解码。实时语音场景下这个代价完全不可接受——这是语音要用 WebRTC 而非 SSE 的直接原因之一。
...
每条连接建立时被负载均衡器路由到某台后端服务器，之后这个用户的所有消息都必须发到**同一台**——因为连接状态就存在那里。

想横向扩容加新机器时，新机器接不到老连接，老连接的用户也无法迁移，扩容效果大打折扣。

常见解法是把连接状态外移到 Redis，通过发布订阅把消息中转到正确的服务器。但架构明显变复杂，多了一跳，延迟也增加。

相比之下，**SSE 的每次请求都是普通 HTTP，负载均衡器可以任意路由**，横向扩展非常简单——这是 SSE 在大规模部署上的明显优势。
...
| **延迟** | 受 TCP 重传影响 | 50–500ms | 50–150ms |
| **丢包处理** | 强制重传 | 强制重传，后续数据等待 | 丢包隐藏，不阻塞 |
| **音视频** | 无，需 Base64 | 无，需自建全套处理链路 | 原生（AEC/NS/AGC/ABR） |
| **建连复杂度** | 极低（普通 GET） | 低（HTTP Upgrade） | 高（信令 + ICE 穿透） |
| **横向扩展** | 简单（无状态） | 麻烦（有状态，需状态外移） | 需 STUN/TURN 服务 |
...
**大原则：单向推就用 SSE，真正需要双向才上 WebSocket，涉及音视频才上 WebRTC。**

OpenAI 和 Anthropic 的文字 API 都用 SSE 而非 WebSocket——绝大多数 LLM 文字产品 SSE 就够了。

## 13.9 常见错误

### 13.9.1 认为「WebSocket 功能更强所以更好」

它们不是「简单 vs 复杂」的关系，是**方向**的差异。WebSocket 的全双工能力换来的是有状态、扩展麻烦、代理不友好。用不到双向就是纯负担。

### 13.9.2 认为 TCP 重传对语音也是好事

这是 WebRTC 那道题最大的雷。丢一个 20ms 音频帧，TCP 强制等重传会把后面所有音频堵住，延迟累积导致通话卡死。**语音容忍丢包，绝不容忍延迟。**

### 13.9.3 认为 WebRTC 的优势是 P2P

P2P 只是它的一个特点，不是核心原因。核心是**UDP + 丢包隐藏 + 内置音频处理链路**。而且大量场景下 WebRTC 根本没走 P2P（打洞失败退到 TURN、或者对端本来就是服务器）。

### 13.9.4 认为 WebRTC 可以完全替代 WebSocket

WebRTC 建连**需要一个信令通道来交换 SDP**，这个通道通常就是 WebSocket。两者是配合关系。

### 13.9.5 只说「SSE 是单向的」就完事

要能说出四个具体局限：双通道架构复杂、HTTP/1.1 六连接上限、只能传文本（Base64 膨胀 33%）、断线重连需 `Last-Event-ID` 支持否则丢内容。

### 13.9.6 忽略 WebSocket 有状态的扩展代价

这是生产环境最实际的痛点：连接绑死在某台机器上，扩容时新机器接不到老连接，要靠 Redis 做状态外移，架构复杂度和延迟都上去了。

## 13.10 本章总结

1. **三者都在解决 HTTP「一问一答」做不到的事**：服务端主动推数据；
2. **SSE 不是新协议**，是 HTTP/1.1 的既有特性，响应体无限长，浏览器有 `EventSource` 原生支持；
3. **文字场景 TCP 是朋友**：token 丢了或乱序意思就变了，你愿意等重传；
4. **SSE 的四个局限**：双通道架构、HTTP/1.1 六连接上限、只能传文本、重连需 `Last-Event-ID`；
5. **WebSocket 的三个局限**：有状态难扩展、企业代理常拒绝 Upgrade、无内置请求-响应配对；
...
10. **选型三句话**：单向推用 SSE，需要双向上 WebSocket，涉及音视频上 WebRTC。
