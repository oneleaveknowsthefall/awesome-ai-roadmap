...
要理解这三者，得先回到一个更底层的问题：**它们在解决什么 HTTP 做不到的事？**

传统 HTTP 请求由客户端发起，服务端在该响应中返回数据；HTTP 流式响应、SSE、长轮询与 HTTP/2/3 流可让服务端在一个已建立的响应中持续发送数据，但不能凭空向未建立的客户端请求推送。

这在传统 Web 里够用，但 AI 场景不行：模型生成一个完整回答需要几秒到十几秒，等全部生成完再一次性返回，用户只能干瞪着空白屏幕。我们要的是**生成一个词推一个词**，像 ChatGPT 那样逐字「打」出来。

这需要让响应保持打开并持续发送数据；SSE 是对此类 HTTP 流式场景的标准化封装之一。
...
不是 JSON 包装，就是纯文本：每条消息以 `data:` 开头，结尾两个换行符。
...
从这一刻起，这条 TCP 连接「变性」了——不再遵循 HTTP 的一问一答，变成**双方都可随时说话的全双工信道**。
...
SSE 只能服务端→客户端，用户发消息必须走独立 POST。**同一个对话用了两条通道**，靠 conversation ID 关联。

服务端收到 POST 后要找到对应的 SSE 长连接把输出推过去。简单场景够用，但状态管理比 WebSocket 的单通道复杂，出问题时排查链路更长。

> 这和 [第十二章](12-mcp-transport.md) 里 MCP 废弃「HTTP + SSE 双端点」的理由是同一个问题。
...
SSE 的事件字段是 UTF-8 文本。二进制媒体通常须 Base64 编码、以 URL/文件引用传递，或改用其他通道；Base64 会增加传输与编解码成本。它通常不适合作为低延迟连续媒体传输，但是否可接受取决于数据量和时延目标。
...
SSE 不会有这个问题——它始终是普通 HTTP 请求，任何代理都能透传。
...
> JSON-RPC 2.0 的 `id` 字段解决的就是这个问题——见 [第十二章](12-mcp-transport.md)。
...
这是用 WebSocket 传语音最难补齐的部分，也是很多人只盯着传输协议而忽略的差异：
...
综合来看：**TCP 系方案（WebSocket）在网络抖动时达不到延迟要求，且没有内置音频处理能力**，需要大量额外工程。WebRTC 天然满足所有要求。
...
**选型原则**：单向事件流常选 SSE；需要应用层全双工消息时评估 WebSocket；实时交互式音视频通常评估 WebRTC。代理、浏览器、媒体处理与运维约束同样会改变选择。
...
> **一句话概括：文字要的是「一个字都不能错」，所以走 TCP 上的 SSE；语音要的是「一秒都不能等」，所以走 UDP 上的 WebRTC——两条相反的取舍，各自服务于完全不同的用户体验。**
