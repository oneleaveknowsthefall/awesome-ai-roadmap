...
函数签名和 docstring 会被自动转成 JSON Schema。docstring 就是模型看到的 `description`——[第三章](03-tool-schema-design.md) 那套写法在这里同样适用。
...
最后两个在生产环境影响最大。上下文膨胀的解法是 [第三章](03-tool-schema-design.md) 讲的动态工具筛选——不要把所有 Server 的工具无脑全量注入。
...
装一个第三方 MCP Server 等于在应用里跑第三方代码，它能看到所有传给它的参数。工具描述本身也可能被投毒。见 [Agent 安全章节](../agent/15-agent-security.md)。
