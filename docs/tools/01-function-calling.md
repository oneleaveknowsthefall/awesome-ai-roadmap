...
- **一种输出约定**。它是模型层的接口协议，不涉及工具怎么被发现、怎么被分发、怎么跨进程通信——那是 [MCP](04-what-is-mcp.md) 的事。
...
模型看到这条会自己纠正城市名重试。如果你直接抛异常中断整个流程，就浪费了模型的自我修复能力。这一点在 [Agent 的反思机制](../agent/12-agent-reflection.md) 里会展开。
...
| 未解决的问题 | 由谁解决 |
|---|---|
| 工具怎么被**发现**（不用硬编码在代码里） | [MCP](04-what-is-mcp.md) |
| 工具怎么**跨进程 / 跨机器**提供 | [MCP 传输层](12-mcp-transport.md) |
| 复杂任务的**操作方法**怎么复用 | [Skill](08-what-is-skill.md) |
