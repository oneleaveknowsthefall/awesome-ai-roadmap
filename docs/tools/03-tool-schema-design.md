...
这本质上是把 few-shot 塞进了工具定义里。代价是 token，收益是参数准确率，值不值得取决于这个工具被调用的频率和出错的成本。
...
这是最容易被低估的成本。工具 Schema 不会被「记住」，每一轮请求都要完整传一遍：
...
> **一句话概括：工具 Schema 不是给机器看的接口定义，而是给模型看的说明书，它的措辞质量直接决定了 Agent 的行为质量。**
