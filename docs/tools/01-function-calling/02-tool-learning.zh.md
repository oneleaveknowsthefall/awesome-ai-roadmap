...
不必等基础设施完备才开始测。一种可行的最小做法是：**从 2.2.2 的五类场景中各取 4–6 条真实业务问题，组成一个 20–30 题的固定评测集**，每条标注期望行为（该调用哪个工具、参数是什么，或“不该调用”）。

| 步骤 | 做什么 | 注意 |
| --- | --- | --- |
| 1. 出题 | 每类场景 4–6 题，含约三分之一“不该调用”的题 | 题目应来自真实业务，而非训练用过的样例 |
| 2. 标注期望 | 写明正确的调用与参数，或写明“直接回答” | 标注分歧先解决再进集，否则评测结果缺乏可比性 |
| 3. 固定环境 | 锁定模型快照、chat template、工具 Schema 和执行代码 | 换任一环节都相当于换了被测对象 |
| 4. 跑分并分项记录 | 按 2.7 节列出的维度分别统计，不只看总分 | 工具选对但参数错的样例要单独归类 |
| 5. 回归使用 | 每次改提示、换模型或改 Schema 后重跑 | 分数下降时先查运行时改动，再怀疑模型 |

这个规模不足以支撑论文级结论，但足以暴露过度调用、参数构造错误和运行时适配问题；规模扩大时应优先增加场景多样性，而不是对同一类问题简单复制。
