...
| **Temperature 调低** | 随机偏差型幻觉 | 只能解决「模型本来知道但随机抽到了低概率错答案」。**事实性问答用 0–0.3 是工程标配**（见 [第十三章](13-temperature-top-p-top-k.md)） |
...
**回到最开始那句话**：LLM 的本质是按概率续写下一个 token，**这个机制天然就带着「会编」的可能性**。
...
> **能说出「幻觉不可能消除，工程目标是降低发生率 + 让用户识别」，就已经比绝大多数人深刻一个层次。** 这个认知背后是对 LLM 概率本质的理解。
...
> **一句话概括：幻觉不是模型的 bug，而是「按概率续写」这个设计的必然副产物——所以真正的工程问题从来不是怎么消灭它，而是怎么把它压到可接受范围并让用户看得出来。**
