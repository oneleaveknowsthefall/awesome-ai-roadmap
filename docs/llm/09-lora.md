...
不动原始权重 $W$，在旁边加两个小矩阵 $A$ 和 $B$，训练时只更新它们，$W$ 全程冻结。

$$
h = x W + \frac{\alpha}{r} \, x B A
$$
...
| 矩阵 | 形状 | 作用 |
|---|---|---|
| $A$ | $r \times d$ | 把输入从 $d$ 维压缩到 $r$ 维 |
| $B$ | $d \times r$ | 把 $r$ 维还原回 $d$ 维 |
| $BA$ | $d \times d$ | 和原始更新矩阵同维，**但参数量大幅减少** |
...
W_merged = W + alpha * (B @ A)   # 只做一次

# 推理时和原始模型完全一样，没有额外计算
output = x @ W_merged
```

训练完成后把 $\alpha BA$ 加到 $W$ 上得到 `W_merged`，**推理时计算图和原始模型完全相同**，不需要带着 $A$、$B$。
...
**这就是它成为 PEFT 事实标准的原因——不是单一维度领先，而是五个优点叠加。**
...
5. **推理零开销**：$\alpha BA$ 可提前合并进 $W$，计算图与原模型完全一致——**这是它胜过 Adapter 的关键**；
...
10. **成为 PEFT 事实标准的原因是五个优点叠加**，而不是单点领先。
