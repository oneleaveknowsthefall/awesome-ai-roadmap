...
>
> **更稳的做法**：让模型内部先分析，最终只输出简洁依据、关键步骤或可核查的结论。
...
**Prompt 工程本质是「提出假设 → 测试 → 优化」的循环，不是一次写完就完事的。**
...
> **一句话概括：写 Prompt 的核心不是把话说得漂亮，是把模型的「自由发挥空间」一层层收窄到你真正想要的那个范围里，然后用测试集把这个过程工程化。**
