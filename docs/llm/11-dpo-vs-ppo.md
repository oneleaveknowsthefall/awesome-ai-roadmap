...
> [第十章](10-post-training.md) 横向梳理了五类 Post-Training 方法。本章聚焦其中最常被拿来对比的两个——PPO 和 DPO——把它们的机制差异、权衡边界和选型逻辑讲透。
...
这个类比能让面试官立刻抓住两者的结构差异，比纯讲算法生动得多。
...
**能把这四个分别做什么讲清楚，是这道题的关键分水岭**——尤其 value model，不知道它干什么就理解不了 GRPO 的创新点。
...
能讲出「**砍掉 Value Model 用组内归一化代替**」这一句，就是高分回答。
...
> **一句话概括：PPO 和 DPO 的分水岭不在「谁效果好」，而在「模型能不能自己去试没试过的回答」——DPO 用这个探索能力换来了砍掉一半模型和一整套 RL 基础设施。**
