...
它覆盖 SFT 之后所有继续训练的方法。下面五大类是工业界最主流的方案，**每一类的设计哲学都不一样**。
...
由 DeepSeek 在 DeepSeek-Math 论文里提出，后来 DeepSeek-R1 把它推向风口浪尖。**现在问对齐方法，GRPO 几乎是必问题。**
...
> **一句话概括：Post-Training 这条线的演进主轴就是「在保住 RL 探索能力的前提下，一个个砍掉需要维护的模型」——RLHF 四个，GRPO 三个，DPO 两个，拒绝采样干脆退回监督学习。**
