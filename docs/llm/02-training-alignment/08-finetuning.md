---
description: 比较全量微调、参数高效微调和指令微调的适用场景，说明数据、算力、稳定性与部署成本之间的取舍。
---
...
> 把「改哪些参数」和「学什么目标」分开看，LoRA、QLoRA、SFT、DPO 才不会混成一团。
...
成本、调参难度、维护成本都会指数级上升。大多数团队最后输的，都是收益撑不起额外复杂度。
...
> 先判断需求到底该不该微调，再区分参数方案和训练目标；多数项目从 QLoRA + SFT 起步就够了。
