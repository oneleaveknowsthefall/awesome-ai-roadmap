...
**没有推翻，是优化目标不同**。Chinchilla 优化训练算力最优，Llama 3 优化推理成本最优。把这点讲清楚，才不会把两条路线误判成互相否定。
...
数据墙 + 算力墙，以及合成数据、多模态、RL 这三个应对方向。后续路线为什么转向这些方向，基本都能从这两堵墙推出来。
...
> Scaling Law 解决的是「投入大致能换来多少 loss 改善」，而 Chinchilla 到 Llama 3 的变化解决的是「同样的预算该花在参数、数据还是部署成本上」。
