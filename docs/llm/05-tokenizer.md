...
**不会出现 OOV**，每个部分都是有意义的 token。最坏情况下退化到字符级，但绝不会「处理不了」。

这也是「Byte」这个词的含义：以字节为最小单位，**任何 UTF-8 文本都能被表示**，包括从未见过的表情符号和小语种。
...
如果文档恰好卡在上下文边缘，Tokenizer 可能把一个词从中间硬切开（「人工智能」被截成「人工」+ 半个字），导致下游解析或检索失败。
...
原因就在 tokenizer——`strawberry` 可能被切成 `str` + `aw` + `berry` 这样的块，模型看到的是几个整体符号，**根本看不到里面有几个字母 r**。
...
应该在语义边界截断并留安全 buffer，否则会把词从中间切开。
...
5. **BPE 解决 OOV 的方式**是最坏退化到字符级，任何 UTF-8 文本都能表示；
