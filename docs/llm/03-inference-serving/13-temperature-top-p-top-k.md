...
> [第十二章](12-decoding-strategies.md) 把解码策略的全貌讲清楚了。本章聚焦最常被调、也最常被调错的三个采样参数。
...
关键是它能**根据分布形状自动调整候选池大小**：确定性问题池小、开放性问题池大。这一句能把回答提升一个层次。
...
> **一句话概括：Temperature 决定分布有多松，Top-P 决定从中截多少——真正的调参功力不在于知道它们是什么，而在于克制到一次只动一个。**
