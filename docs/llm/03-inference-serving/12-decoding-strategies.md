...
三者的详细调参见 [第十三章](13-temperature-top-p-top-k.md)。**采样族通过「引入随机性 + 截断长尾」，在多样性和质量之间找平衡。**
...
**为什么能加速？** 大模型推理的瓶颈是**访存**——每次只生成一个 token，却要把整个权重从显存搬到计算单元（详见 [第三章](../01-foundations/03-attention-variants.md) 的 memory-bound 分析）。如果一次能验证 5 个 token，**访存次数就少了 5 倍**。
